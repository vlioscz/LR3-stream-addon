"""Render the brand PNGs for the LR3 Stream add-on.

Draws the same design as icon.svg with Pillow -- shutter slats behind a red LR3
wordmark with vlios.cz beneath -- and writes the four files: a square icon and a
wider logo, each at 1x and 2x. Then copies icon.png and logo.png into the add-on
folder so Home Assistant shows them in the add-on store.
...
def render_icon() -> Image.Image:
    """The square icon: slats, LR3, vlios.cz, trimmed to fill the square."""
    size = 512
    ...
    _text(draw, (256, 250), "LR3", _font(190), RED, spacing=6)
...
def render_logo() -> Image.Image:
    """The wide logo: LR3 with vlios.cz beneath, for the add-on page."""
    w, h = 720, 400
    ...
    _text(draw, (360, 200), "LR3", _font(230), RED, spacing=8)
...
def main() -> None:
    """Render every brand file and copy the 1x assets into the add-on."""
    print("Rendering brand assets:")
    _save(render_icon(), "icon", 256)
    _save(render_logo(), "logo", 256)
    for name in ("icon.png", "logo.png"):
        shutil.copyfile(OUT / name, ADDON / name)
        print(f"  copied {name} -> {ADDON.name}/{name}")
